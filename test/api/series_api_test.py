...
from ..types.message_success_result_stub import MessageSuccessResultStub
from ..types.metric_summary_stub import MetricSummaryStub
...
from ..types.timeseries_query_response_stub import TimeseriesQueryResponseStub

MODELS_AVAILABLE = (
    find_spec("waylay.services.data.models") is not None
)
...
def _delete_series_set_mock_response(
    ...
    mock_response = MessageSuccessResultStub.create_json()
    httpx_mock_kwargs = {
        "method": "DELETE",
...
async def test_delete_series(
    ...
    check_type(resp, MessageSuccessResult)
...
def _get_series_set_mock_response(
    ...
    mock_response = [MetricSummaryStub.create_json()]
    httpx_mock_kwargs = {
        "method": "GET",
        "url": re.compile(f"^{gateway_url}/data/v1/series/{resourceId}(\\?.*)?"),
        "content": json.dumps(mock_response, default=str),
...
async def test_get_series(
    ...
    check_type(resp, list[MetricSummary])
...
def _query_time_series_set_mock_response(httpx_mock: HTTPXMock, gateway_url: str):
    mock_response = TimeseriesQueryResponseStub.create_json()
    httpx_mock_kwargs = {
        "method": "POST",
...
async def test_query_time_series(
    ...
    check_type(resp, TimeseriesQueryResponse)
